...
1. Vite bundles React app to IIFE format (Immediately Invoked Function
   Expression)
...
5. Auth token (if needed) is read from
...
**Strategy:** Use Tailwind CSS - all styles are automatically inlined in the
JavaScript bundle.
...
- Next.js app at `widgets.yourchurch.org/event-registration` (dev preview, API
  routes)
...
5. **Embed:** Use `https://widgets.yourchurch.org/widget/event-registration.js`
   in WordPress

---

**Status:** Template not yet built **Next Steps:** Build template when first
WordPress integration is needed **Pattern:** Next.js for development + Vite for
production bundle **Examples:** See `NextJS/Widgets/RSVP/` and
`NextJS/Widgets/Prayer/` for working implementations
