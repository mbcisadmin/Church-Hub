...
MinistryPlatform requires audit logging for stored procedure calls. Every stored
procedure that modifies data requires a `$userId` parameter to track who made
the change.
...
✅ **Compile-time safety** - Can't forget `$userId` ✅ **Clear errors** -
TypeScript tells you exactly what's missing ✅ **Impossible to bypass** - No way
to call service without userId ✅ **Self-documenting** - Types show what's
required ✅ **Runtime validation** - Zod validates `$userId` is a number ✅
