...
#### a) User Auth Procedure

This procedure returns user profile, admin status, and User Groups in a single
call. It checks for a specific Security Role (by ID) to determine admin status,
and returns only User Groups (not security roles) since security roles cost
extra money.
...
CREATE OR ALTER PROCEDURE [dbo].[api_Custom_GetUserRolesAndGroups_JSON]
    ...
    @AdminRoleID INT = NULL,  -- Optional: Security Role ID for admin check
    @DomainID INT = 1
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT @UserID = u.User_ID, @ContactID = u.Contact_ID
    FROM dp_Users u WHERE u.User_GUID = @UserGUID;

    IF @UserID IS NULL
    BEGIN
        SELECT '{"User":null,"IsAdmin":false,"Groups":[]}' AS Result;
        RETURN;
    END

    -- Get contact details
    SELECT @FirstName = c.First_Name, @Nickname = c.Nickname,
           @LastName = c.Last_Name, @Email = c.Email_Address,
           @MobilePhone = c.Mobile_Phone
    FROM Contacts c WHERE c.Contact_ID = @ContactID;

    -- Get profile image
    SELECT TOP 1 @ImageGuid = CAST(f.Unique_Name AS NVARCHAR(100))
    FROM dp_Files f
    WHERE f.Table_Name = 'Contacts' AND f.Record_ID = @ContactID AND f.Default_Image = 1
    ORDER BY f.File_ID DESC;

    -- Check admin security role
    IF @AdminRoleID IS NOT NULL AND EXISTS (
        SELECT 1 FROM dp_User_Roles ur WHERE ur.User_ID = @UserID AND ur.Role_ID = @AdminRoleID
    ) SET @IsAdmin = 1;

    -- Get User Groups
    DECLARE @GroupsJson NVARCHAR(MAX) = '';
    SELECT @GroupsJson = @GroupsJson +
        CASE WHEN @GroupsJson = '' THEN '' ELSE ',' END +
    ...
    WHERE uug.User_ID = @UserID ORDER BY ug.User_Group_Name;
...
**Find your Admin Security Role ID:**

```sql
-- Find the Role_ID for your admin security role
SELECT Role_ID, Role_Name FROM dp_Roles WHERE Role_Name LIKE '%Hub%';
-- or
SELECT Role_ID, Role_Name FROM dp_Roles WHERE Role_Name = 'TheHub';
```

Add this Role_ID to your `.env` file as `ADMIN_SECURITY_ROLE_ID`.
...
EXEC [dbo].[api_Custom_GetUserRolesAndGroups_JSON]
    ...
    @AdminRoleID = 123;  -- Your admin role ID
```
...
3. Add:
   ...
   - **Description:** "Get user profile, admin status, and groups for
     authentication"
4. Save
...
1. Edit `apps/platform/src/config/church.ts` — all church identity lives here
2. The PWA manifest, page titles, and footer are all driven by `church.ts`
