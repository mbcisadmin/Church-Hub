...
export default function NavigationSidebar({ open, onClose }: NavigationSidebarProps) {
  ...
  }, [open, pathname, getActiveSectionId]);

  // Fetch apps when sidebar opens
  useEffect(() => {
    if (!open) return;
        ...
        {open && (
          <>
            ...
            <motion.aside
              ...
              className="bg-secondary fixed inset-y-0 left-0 z-[60] flex w-[280px] max-w-[85vw] flex-col shadow-xl"
            >
              {/* Header */}
              <div className="px-4 pt-3 pb-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  >
                    ...
                    <span className="group-hover:text-primary text-sm font-medium transition-colors duration-300">
                      Close
                    </span>
                  </button>
                  <button
                    onClick={onClose}
                    className="hover:bg-primary/20 flex h-8 w-8 items-center justify-center text-white/70 transition-colors hover:text-white"
                    aria-label="Close menu"
                  >
                    <X className="h-5 w-5" />
                  </button>
              ...
              <div className="relative flex-1">
                ...
                >
                  {/* Filter input — inside scroll container for alignment */}
                  ...
                  {isLoading ? (
                    ...
                    <div className="flex min-h-full flex-col">
                      <nav className="space-y-4">
                        ...
                        {(() => {
                          let primaryList = filterSections(
                            PRIMARY_SECTIONS
                          ) as typeof PRIMARY_SECTIONS;
                          if (pinnedOnly) {
                          ...
                          return primaryList.map((section) => {
                            ...
                            return (
                              <div key={section.id} data-section-id={section.id}>
                                <div
                                  className={`mb-2 flex w-full items-center gap-2 px-2 text-[13px] font-semibold tracking-wider uppercase ${isActive ? 'text-primary' : 'text-white/40'}`}
                                >
                                  <button
                        ...
                        {/* Dynamic Sections - alphabetized, filtered by access level */}
                        {(() => {
                          ...
                          if (
                            dashboards.length > 0 &&
                            visibleDynamicSections.includes('analytics')
                          ) {
                            dynamicList.push({
                              id: 'analytics',
                              label: 'Analytics',
                          ...
                          // Sort alphabetically (mobile doesn't bump active to top)
                          dynamicList.sort((a, b) => a.label.localeCompare(b.label));
                                        ...
                                        >
                                          ...
                                          {section.addAction && (
                                            <AddActionButton
                                              label={section.addAction.label}
                                              onClick={() =>
                                                handleNavigate(section.addAction!.route)
                                              }
                                            />
                                          )}
                                        </motion.div>
                      ...
                      {/* Bottom — pin filter + logo */}
                      <div className="mt-auto px-1 pt-5 pb-2">
                        ...
                        <div className="flex items-center justify-between px-1">
                          ...
                          >
                            <Pin
                              className={`h-5 w-5 transition-colors ${pinnedOnly ? 'text-primary' : 'text-white/30 group-hover:text-white/70'}`}
                            />
                            <span
                              className={`text-xs font-medium transition-colors ${pinnedOnly ? 'text-primary' : 'text-white/30 group-hover:text-white/70'}`}
                            >
                              Pinned
                            </span>
                          </button>
                          <ChurchLogo className="h-6 w-6 shrink-0 text-white/[0.06]" />
                        </div>
                      </div>
                    </div>
                  )}
...
function NavItem({
  ...
  return (
    <button onClick={onClick} className={`${baseClasses} ${variantClasses[variant]}`}>
      ...
      <span
        className={`flex-1 truncate text-left text-[13px] font-medium transition-colors duration-300 ${
          variant === 'danger' ? 'group-hover:text-red-300' : 'group-hover:text-primary'
        }`}
      >
        {label}
...
function SubNavItem({
  ...
  return (
    <button
      onClick={onClick}
      className={`group flex w-full items-center gap-1.5 rounded-md py-0.5 pr-2 text-left text-[13px] font-medium tracking-wide uppercase transition-colors ${
        isNested ? 'pl-8' : 'pl-5'
...
function AddActionButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="group text-primary/70 hover:text-primary mt-1 flex w-full items-center gap-1.5 rounded-md py-0.5 pr-2 pl-5 text-left text-[13px] font-medium tracking-wide transition-colors"
    >
