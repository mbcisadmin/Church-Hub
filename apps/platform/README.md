...
This is the main apps platform that hosts multiple micro-apps under a single domain (e.g., `apps.church.org`). Each micro-app is independently installable as a PWA.
...
- **Protection:** Middleware protects all routes except `/signin` and `/api/auth`
...
4. **Add to navigation:**
   Update `src/app/(app)/layout.tsx` to include link

5. **Configure as PWA:**
   Each micro-app can have its own manifest and be installed independently
